...
//findViewById( R.id.etPlain)
        ...
        if (bundle != null){
            String decryptedMessage = bundle.getString( THE_MESSAGE );
            int rotation = bundle.getInt( THE_ROTATION );
            if (DEBUG) {

                Log.i(LOG_TAG, "ENCRYPTING decrypted Message (rotation: " + rotation + "): " + decryptedMessage);
            }
            mEncryptedText.setText(CaesarCipher.encrypt(decryptedMessage, rotation));
        }
